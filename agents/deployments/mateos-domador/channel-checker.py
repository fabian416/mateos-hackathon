...
def send_attachments_to_telegram(attachments, subject=""):
    ...
    for att in attachments:
        ...
        size = att["size"]
        caption = f"\U0001f4ce {filename}" if not subject else f"\U0001f4ce {filename} \u2014 {subject}"
    ...
    if failed:
        msg = "\U0001f4ce Algunos adjuntos no se pudieron reenviar:\n\n" + "\n".join(failed)
        notify_telegram(msg)
...
def send_proposal_to_telegram():
    ...
    # Channel indicator
    channel_emoji = "\U0001f4e7" if channel == "email" else "\U0001f4ac"
    channel_label = "Email" if channel == "email" else "WhatsApp"

    att_line = ""
    if attachments:
        names = ", ".join(a.get("filename", "?") for a in attachments)
        att_line = f"\n\U0001f4ce Adjuntos: {len(attachments)} archivo(s) ({names}) \u2014 enviados aparte\n"
    ...
    subject_line = f"\n\U0001f4cc Asunto: {safe_subject}" if subject else ""
    ...
    proposal = (
        ...
        f"Respond\u00e9: \u2705 enviar | \u270f\ufe0f modificar | \u274c descartar | \U0001f5d1\ufe0f olvidar"
    )
...
def handle_completed_state():
    ...
    if action == "sent":
        ...
        notify_telegram(f"\U0001f5d1\ufe0f {channel_emoji} Olvidado \u2014 {subject}")
    else:
